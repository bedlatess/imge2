...
一个面向商业化使用的 AI 图像生成站，支持文生图、图生图、提示词库、账号系统、个人连接、管理员工作区连接和浏览器本地使用记录。
...
- 连接凭证加密保存
- 浏览器本地使用记录与生成历史，默认不把提示词和生成记录写入服务器
...
账号、会话和加密后的连接凭证会保存在服务器项目目录的 `data` 文件夹中。生成图片和使用记录默认保存在当前浏览器本地，不会写入服务器。

如果旧版本已经产生过服务器端使用记录，登录后可通过旧版清理接口或数据库备份策略自行清理 `usage_logs` 表。
